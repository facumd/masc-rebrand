...
import { TipoUsuario, Usuario } from '../models/user.interface';
import { ResultadoApi } from '../models/response.interface';
import { HttpClient } from '@angular/common/http';
import { environment } from 'src/environment/environment';
import { TokenUsuario } from './user.service';
...
export class AuthService {
  ...
  public getUserPermissions(): TipoUsuario {
    return this.usuario?.permissions ?? TipoUsuario.Invitado;
  }
  ...
  login(usuario: string, clave: string): Observable<ResultadoApi> {
    ...
    return this.http.post<ResultadoApi>(this.loginUrl, formData).pipe(
      tap((resultado: ResultadoApi) => {
        let tokenUsuario = resultado.data as TokenUsuario;
        let usuarioActual = JSON.stringify(tokenUsuario.usuarioActual);

        localStorage.setItem(
          'accessToken',
          `${tokenUsuario.accessToken.acceso}`
        );
  ...
  logout(): Observable<ResultadoApi> {
    return this.http
      .post<ResultadoApi>(this.logoutUrl, {})
      .pipe(
        catchError((error) => {
          const resultado: ResultadoApi = {
            mensaje: error.error.mensaje,
